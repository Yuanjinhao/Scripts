...
Name  : GetHeatMap.py
Author: yuan.jinhao
Time  : 2024/10/14 下午12:07
Desc  : 绘制热力图脚本，可以对指定网络层绘制热力图
"""
import warnings
...
class yolov8_target(torch.nn.Module):
    def __init__(self, ouput_type, conf, ratio) -> None:
        ...
        self.conf = conf
        self.ratio = ratio
    # post_result为预测类别，pre_post_boxes为预测的box，其已根据置信度从大到小排列。
...
class yolov8_heatmap:
    def __init__(self, weight, device, method, layer, backward_type, conf_threshold, ratio, show_box, renormalize):
        ...
        colors = np.random.uniform(0, 255, size=(len(model_names), 3)).astype(np.int8)
    ...
    def __call__(self, img_path, save_path):
        ...
        if os.path.isdir(img_path):
            for img_path_ in os.listdir(img_path):
                file_name = os.path.basename(img_path_ )
                self.process(f'{img_path}/{img_path_}', f'{save_path}/{file_name}')
        else:
            file_name = os.path.basename(img_path)
            self.process(img_path, f'{save_path}/{file_name}')


def get_params():
    params = {
        'weight': './weights/yolo11s.pt',  # 现在只需要指定权重即可,不需要指定cfg
        'device': 'cpu',
        'method': 'GradCAM',
